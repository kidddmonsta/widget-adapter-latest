var updateWidget = function (widgetJson) {
    var query = {};
    query.widgetOptions = widgetJson.widgetOptions;
    query.widgetOptions.storage = localStorage.getItem('storage');

    $.ajax({
        type: 'PATCH',
        url: IP_TO_WIDGET_CORE + widgetJson.id,
        data: JSON.stringify(query),
        processData: false,
        contentType: 'application/merge-patch+json',
    }).done(function (widgetData) {
        console.log(widgetData);
        alert("Настроойки сохранены");
    });
};
document.onkeyup = function (e) {
    if (e.ctrlKey && e.altKey && e.which == 89) {
        updateWidget(widgetJson);
        return false;
    }
};
...
function render(blockId) {
    prepareData().then(function () {
        $.getJSON('widgetOptions.json').then(function (widgetOptions) {
            ...
            if ('storage' in widgetJson.widgetOptions !== false) {
                console.log("storage is setted");
                console.log(JSON.parse(widgetJson.widgetOptions.storage));
                localStorage.setItem('storage', widgetJson.widgetOptions.storage);
            }
